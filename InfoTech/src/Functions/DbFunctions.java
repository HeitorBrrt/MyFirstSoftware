...
public class DbFunctions {
    ...
    public void createTables(Connection conn){
        ...
        try {
            ...
            String queryCreateTableVenda = "CREATE TABLE IF NOT EXISTS Venda (idVenda int PRIMARY KEY DEFAULT nextval('incremento'), descricao text, fk_Cliente_idCliente int, fk_Usuario_usuario text, precoTotal float, dataVenda text);";
            statement = conn.createStatement();
    ...
    public void insert_row_produto(Connection conn, String table_name, String name, String number, String address,
    ...
    public void insert_row_venda(Connection conn, String table_name, String descricao, Integer idCliente,
                                 String userUsuario, String precototal, String datavenda){
        Statement statement;
        try {
            String query = String.format("INSERT INTO %s(descricao, fk_cliente_idCliente, fk_usuario_usuario, precototal, datavenda) VALUES ('%s', '%s', '%s', '%s', '%s');", table_name, descricao, idCliente, userUsuario, precototal, datavenda);
            statement=conn.createStatement();
            statement.executeUpdate(query);
    ...
    public void vendaEfetuada(Connection conn, String usuario, String qtdVendas){
        Statement statement;
        try{
            String query = String.format("UPDATE usuario SET qtdvendas = '%s' WHERE usuario  = '%s'", qtdVendas, usuario);
            statement=conn.createStatement();
            statement.executeUpdate(query);
        } catch (Exception e) {
            JOptionPane.showMessageDialog(null, e);
    ...
    public List<Venda> read_dataVendas(Connection conn){
        ...
        try {
            ...
            while (rs3.next()){
                ...
                venda.setQtdItem(String.valueOf(rs3.getString("")));
    ...
    /*    public String getPrecoVenda(Connection conn, Integer idproduto) {
        Statement statement;
        ResultSet rs;
        try{
            String query = String.format("SELECT precovenda FROM produto WHERE idproduto='%s';", idproduto);
